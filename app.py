    ...
    with tabs[0]:
        ...
        if df.empty:
            ...
            if df_filtered.empty:
                ...
                latest_filtered = df_filtered.sort_values('date').groupby('student_id').lasimport streamlit as st
import sqlite3
import pandas as pd
import plotly.express as px
from datetime import datetime
...
# ---------- Базаны толығымен қайта құру (ескісін жою) ----------
def reset_and_init_db():
    # Ескі базаны жою
    if os.path.exists(DB_NAME):
        os.remove(DB_NAME)
        print(f"Ескі {DB_NAME} жойылды.")
...
def insert_sample_data():
    ...
    # 1. Барлық ескі деректерді өшіру (қауіпсіздік үшін, бірақ база жаңа болғандықтан қажет емес)
    c.execute("DELETE FROM activities")
    c.execute("DELETE FROM users WHERE role='student'")
    c.execute("DELETE FROM students")
...
# ---------- Бастапқы іске қосу: ескі базаны жойып, 2026 деректерін құру ----------
reset_and_init_db()
insert_sample_data()


# ---------- Қалған функциялар (өзгеріссіз, тек update_year_to_2026 алып тасталды) ----------
def get_students():
    conn = sqlite3.connect(DB_NAME)
    df = pd.read_sql_query("SELECT id, name FROM students", conn)
    conn.close()
...
with st.sidebar:
    if st.session_state.role == "teacher":
        ...
        # Жылды ауыстыру батырмасы қажет емес, өйткені тек 2026 бар
        st.markdown("---")
        st.markdown("### 📊 Графиктер")
        if st.button("📊 Белсенділік", key="sidebar_act"):
            st.session_state.show_act = True
...
# Мұғалім интерфейсі (өзгеріссіз, бірақ күн аралығы 2026)
if st.session_state.role == "teacher":
    students_df = get_students()
            ...
            try:
                ...
                if 'task_type' in df_import.columns:
                    df_import['task_type'] = df_import['task_type'].replace({
                        'Форматитеті': 'Формативті', 'Форматтвті': 'Формативті', 'формативті': 'Формативті'
                    })
                ...
                if missing_cols:
                    ...
                    for idx, row in df_import.iterrows():
                        ...
                        grade_10 = int(float(row['grade_10']))
